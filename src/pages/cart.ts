...
class Temp extends Template {
  ...
  public createItemBlock(itemsInCartObj: IProductInCart[] | [], page: number): void {
    ...
    function collectElements() {
      ...
      function createCardCont(arg: number): HTMLElement {
        const cardCont = temp.createElement('prod-cont__card-cont', cardsWrapper);
      ...
      function createInfoBlock(dataCont: HTMLElement, title: string, description: string, rating: number, discount: number) {
        ...
        const discWrap = temp.createElement('prod-cont__disc-wrap', addInfoCont);
      ...
      function createControlBlock(cardCont: HTMLElement, priceArg: number, id: number, quantity: number, stock: number) {
        const controlBlock = temp.createElement('prod-cont__contr-block', cardCont);
        const controlsWrap = temp.createElement('prod-cont__contr-wrap', controlBlock);
        ...
        const stockNum = temp.createElement('stock-num', stockBl, String(stock));
  ...
  public createSummary(itemsInCart: IProductInCart[] | []): void {
    ...
    const totalContPromo = this.createElement('sum-prod__total-cont', sumInfoWrap);
    ...
    const appliedPromosBlock = this.createElement('applied-promos-block', sumInfoWrap);
  ...
  public changeAmountInCart(itemsInCart: IProductInCart[] | []): void {
    ...
    const cardsWrapper = document.querySelector('.cards-wrapper') as HTMLElement;
    const headerCart = document.querySelector('.header-bottom__items-amount') as HTMLElement;
    const headerSum = document.querySelector('.header-bottom__total-sum') as HTMLElement;
    ...
    function getClickedItem(event: Event): void {
      ...
      promoCode.applyPromo();
      const totalSumDiscount = document.getElementById('total-sum-discount') as HTMLElement;
      if (totalSumDiscount) {
  ...
  public createArraysForPagination(itemsInCart: IProductInCart[] | []): IProductInCart[][] {
    const rowsInput = document.querySelector('.prod-cont__items-num') as HTMLInputElement;
  ...
  public changePages(): void {
    const btnBlock = document.querySelector('.prod-cont__pages-cont') as HTMLElement;
    const pageEl = document.querySelector('.count') as HTMLElement;
